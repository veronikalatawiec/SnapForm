...
export default function Modal({ formId, onClose, setForms }) {
    ...
    const handleDelete = async () => {
        try {
          ...
          await axios.delete(`${import.meta.env.VITE_API_URL}/forms/${userId}/${formId}`, {
            headers: {
  ...
  return (
    <div className="modal__overlay">
      <div className="modal">
        <h3 className="modal__head">Are you sure you want to delete this form?</h3>
        <p className="modal__text">This action cannot be undone.</p>
        <div className="modal__btns">
            <Button onClick={handleDelete} className="btn--delete" text="Delete" />
            <Button onClick={onClose} className="btn--secondary" text="Cancel" />
        </div>
      </div>
    </div>
  );
